...
<div class=row>
    <div class="col">
        <form action="<?= site_url('kriteria') ?>/periode" method="get">
            <h6>Periode</h6>
            <div class="col-6">
                <fieldset class="d-inline form-group">
                    <select name="id_periode" class="form-select" id="basicSelect">
                        <?php foreach ($periode as $dt_periode): ?>
                            <option
                                <?= $id_periode_now == $dt_periode->id_periode?'selected':null; ?>
                                value="<?= $dt_periode->id_periode; ?>"><?= $dt_periode->periode; ?>
                            </option>
                        <?php endforeach ?>
                    </select>
                </fieldset>
...
<?php if (user_login()->user_level == 'administrator' || user_login()->user_level == 'kasi_kesejahteraan'): ?>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form class="form form-vertical" action="<?= site_url('kriteria/create') ?>" method="post">
                    <input type="hidden" name="id_periode" value="<?= $id_periode_now; ?>">
                ...
                <div class="d-flex align-items-center justify-content-between">
                    <h4 class="card-title"><span class="fa-fw select-all fas me-3"></span>
                        <span class="">Data <?= $title; ?> Periode
                            <?php foreach ($periode as $dt_periode): ?>
                                <?= $id_periode_now == $dt_periode->id_periode ? $dt_periode->periode:null; ?>
                            <?php endforeach ?>
                        </span>
                    </h4>
                    <div>
                        <?php if (count($kriteria) > 0): ?>
                            <?php if (user_login()->user_level == 'administrator' || user_login()->user_level == 'kasi_kesejahteraan'): ?>
                            <a href="<?= site_url('kriteria/generate/'.$id_periode_now); ?>" class="btn btn-sm btn-outline-primary me-3">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="<?= site_url('kriteria/delete/') ?><?= $dt_kriteria->id_kriteria; ?>/<?= $id_periode_now; ?>" class="btn btn-sm btn-outline-danger btn-hapus"><span class="fa-fw select-all fas"></span></a>
